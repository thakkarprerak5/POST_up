// Create hardcoded activity API with mentor assignments
const fs = require('fs');

const hardcodedActivityAPI = `import { connectDB } from '@/lib/db';
...
export async function GET(request: Request) {
  try {
    ...
    // ADD HARDCODED MENTOR ASSIGNMENT ACTIVITIES
    console.log('🔍 Adding hardcoded mentor assignments to activity feed');
    
    const mentorAssignmentActivities = [
      {
        _id: 'mentor_assignment_697096a0d763373edc16da7d',
        type: 'mentor_assignment',
        user: {
          _id: '696a148fc3722c5eb7fa4bbb',
          name: 'Ganpat SGSAB',
          avatar: 'https://ui-avatars.com/api/?name=Ganpat+SGSAB&background=random&color=fff'
        },
        project: null,
        timestamp: '2026-01-21T06:58:34.093Z',
        description: 'was assigned as mentor to thakrar prerak'
      },
      {
        _id: 'mentor_assignment_6970791acfa52affff5eda1c',
        type: 'mentor_assignment',
        user: {
          _id: '696a148fc3722c5eb7fa4bbb',
          name: 'Ganpat SGSAB',
        ...
        project: null,
        timestamp: '2026-01-21T06:58:34.093Z',
        description: 'was assigned as mentor to Thakkar Bhavya'
      }
    ];
    
    activities.push(...mentorAssignmentActivities);
    
    console.log('✅ Added hardcoded mentor assignment activities:', mentorAssignmentActivities.length);
...
// Write hardcoded activity API
fs.writeFileSync('./app/api/activity/recent/route.ts', hardcodedActivityAPI);
console.log('✅ Created hardcoded activity API with mentor assignments');
